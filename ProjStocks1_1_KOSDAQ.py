...
save_name = "itemName_KOSDAQ"

#1. 대신증권 API로 모든 종목 코드 읽어와서 DB에 저장, DB 불러오기

rows = list()

CPE_MARKET_KIND = {'KOSDAQ':2}
...
stockitems.to_sql(save_name, con, chunksize = 1000, if_exists='replace')

print("파일저장완료")

# 저장한 파일 불러오기
df = pd.read_sql("SELECT * FROM itemName_KOSDAQ", con, index_col = None)
print(df.head())
